...
import { QAProduct, QADocument, QASpecifications, ArtworkApproval, SKU, Person, ProductGroup, Location, Vendor, QATemplate, BOMItem, SugarType, LotCode } from '../types';
import { Plus, X, Trash2, Upload, Send, CheckCircle2, AlertCircle, Clock, Image, ChevronDown, ChevronUp, Download, Mail, FileText, ExternalLink, Pencil, Minimize2, Maximize2, Minus, Edit2 } from 'lucide-react';
...
export default function QualityAssurancePage({
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {/* Lot Code Testing Log */}
      <LotCodeTestingLog
        lotCodes={lotCodes}
        sugarTypes={sugarTypes}
        people={people}
        onUpdateLotCodes={onUpdateLotCodes}
      />
    </div>
  );
}
...
/* ─────────────────────────────────────────────────────── */
/*  Lot Code Testing Log – sub-component                  */
/* ─────────────────────────────────────────────────────── */
...
function LotCodeTestingLog({
  lotCodes, sugarTypes, people, onUpdateLotCodes,
}: {
  lotCodes: LotCode[];
  sugarTypes: SugarType[];
  people: Person[];
  onUpdateLotCodes: (lc: LotCode[]) => void;
}) {
