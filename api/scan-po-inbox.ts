...
/**
 * Scheduled PO inbox scan (Vercel Cron, every 3 hours).
 *
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // already exist in the orders table). The every-3-hours cron passes none of these.
  ...
  // window keeps the every-3-hours cron cheap (~8 overlapping runs still cover every
  // attachment email).
  ...
  // run — or the every-3-hours cron — continues where this one left off.
